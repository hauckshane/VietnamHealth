...
vietnam <- vietnam %>% filter(ReaExam == "volunteer") %>% filter(RecExam != "unknow") %>% filter(RecPerExam != "unknow")
```
...
ggarrange(
  ...
  ggplot(proportions_PopularInfo, aes(x = PopularInfo, y = Proportion, fill = PopularInfo)) +
    geom_col(position = "dodge") + labs(title = "Popularity", x = "vietnam", y = "Proportion of People where its been Greater than 12 months",),
